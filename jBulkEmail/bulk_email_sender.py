...
# --- SAAS OTP GATEWAY API KEY ---
# (Leave empty to enable Developer Simulation Mode for passwordless UI testing)
GATEWAY_SENDGRID_KEY = ""
...
try:
    ...
    PIL_SUPPORT = False
    print("Warning: Pillow not installed. Graphical icons on buttons/HUD disabled.")
...
class RoundedButton(tk.Canvas):
    ...
    def __init__(self, parent, text="", command=None, bg="#00E5FF", fg="#000000", 
        ...
        self.image = kwargs.pop('image', None) # Icon Support
    ...
    def draw_button(self):
        ...
        # 3D Shadow Effect (Offset Darker Layer)
        shadow_r = self.corner_radius
        if not self.is_pressed:
            self._draw_rounded_rect(self, w, h, shadow_r, "#000000") # Base shadow
            # Main surface (raised)
            self._draw_rounded_rect(self, w-3, h-3, shadow_r, color)
            
            # Position for text and icon
            center_x, center_y = (w-3)/2, (h-3)/2
            if self.image:
                self.create_image(center_x - 70 if self.text else center_x, center_y, image=self.image, anchor="center")
                self.create_text(center_x + 20 if self.image else center_x, center_y, text=self.text, fill=self.fg, font=self.font)
            else:
                self.create_text(center_x, center_y, text=self.text, fill=self.fg, font=self.font)
        else:
            # Pressed state (lowered - shifted down/right)
            self._draw_rounded_rect(self, w, h, shadow_r, self.active_bg)
...
class BulkEmailSender:
    def __init__(self, root):
        ...
        # Phase 14: Microsoft Compliance - Redirect Writeable Data to %AppData%
        self.user_data_dir = Path(os.environ.get('APPDATA', str(self.base_dir))) / "jBES_Discovery"
        ...
        self.user_dirs = {
            'config':  self.user_data_dir / "config",
            'logs':    self.user_data_dir / "logs",
            'exports': self.user_data_dir / "exports"
        }

        # Legacy Compatibility: Merge into self.dirs for monolith stability
        self.dirs = {**self.app_dirs, **self.user_dirs}

        # Ensure all directories exist
        for d in self.dirs.values(): d.mkdir(exist_ok=True)

        # Phase 14.2: Data Migration Logic
        self._migrate_data()
        ...
        self.root.title("jBES Discovery - Tactical Operations")
        self.root.geometry("1100x800")
        self.root.resizable(True, True)
        self.root.configure(bg='#0B132B')
        
        # Set Window Icon
        icon_path = self.dirs['assets'] / "mission_icon.ico"
        if icon_path.exists():
            try: self.root.iconbitmap(icon_path)
            except: pass
        
        # Singleton Mission Locking Protocol
        ...
        # Phase 0: Auth Gateway
        self.root.after(100, self._check_auth_gate)

    def _check_auth_gate(self):
        """Phase 0: First-Time Login OTP & EULA Validation"""
        if not self.config.get("otp_verified", False):
            self._show_otp_modal()
        elif not self.config.get("eula_accepted", False):
            self._show_eula_modal()

    def _show_otp_modal(self):
        otp_win = tk.Toplevel(self.root)
        otp_win.title("Security Gateway - Live OTP")
        otp_win.geometry("350x220")
        otp_win.transient(self.root)
        otp_win.grab_set()
        
        tk.Label(otp_win, text="First-Time Launch: Hardware Binding", font=("Segoe UI", 12, "bold")).pack(pady=10)
        tk.Label(otp_win, text="Enter your email to receive a secure OTP.", font=("Segoe UI", 8, "italic")).pack()
        
        # Email & Send OTP Frame
        cred_frame = ttk.Frame(otp_win)
        cred_frame.pack(pady=5)
        
        tk.Label(cred_frame, text="Email:").grid(row=0, column=0, sticky="e", padx=5)
        email_entry = ttk.Entry(cred_frame, width=25)
        email_entry.grid(row=0, column=1, pady=2)
        
        import random
        self.current_live_otp = None
        
        def send_otp():
            email = email_entry.get().strip()
            
            if not email or "@" not in email:
                messagebox.showerror("Error", "Please enter a valid email.", parent=otp_win)
                return
                
            self.current_live_otp = str(random.randint(100000, 999999))
            
            if GATEWAY_SENDGRID_KEY:
                try:
                    engine = APIRelayEngine(api_key=GATEWAY_SENDGRID_KEY)
                    engine.send_via_sendgrid(
                        sender_email="noreply.security@gateway.local",
                        to_email=email,
                        subject="Security Gateway - OTP Validation",
                        body=f"Your jBulkEmailSender Gateway OTP is: {self.current_live_otp}",
                        is_html=False
                    )
                    messagebox.showinfo("OTP Dispatched", f"✅ A live 6-digit OTP has been sent to {email} via API!", parent=otp_win)
                except Exception as e:
                    messagebox.showerror("API Error", f"SaaS Relay Failed:\n{e}", parent=otp_win)
                    self.current_live_otp = None
            else:
                # Developer Simulation Mode
                messagebox.showinfo("Simulation Mode", f"Dev Simulation Active (No API Key).\n\nYour OTP is: {self.current_live_otp}\n\n(Enter this code below to proceed).", parent=otp_win)
                
            if self.current_live_otp:
                otp_entry.focus()
            
        send_btn = ttk.Button(cred_frame, text="Send OTP", command=send_otp)
        send_btn.grid(row=1, column=1, pady=5, sticky="e")
        
        # OTP Frame
        otp_frame = ttk.Frame(otp_win)
        otp_frame.pack(pady=10)
        
        tk.Label(otp_frame, text="Enter 6-Digit OTP:").pack(side="left", padx=5)
        otp_entry = ttk.Entry(otp_frame, width=15)
        otp_entry.pack(side="left", padx=5)

        def verify():
            if not self.current_live_otp:
                messagebox.showerror("Error", "You must click 'Send OTP' first.", parent=otp_win)
                return
                
            if otp_entry.get().strip() == self.current_live_otp:
                self.config["otp_verified"] = True
                self.save_config()
                otp_win.destroy()
                self._check_auth_gate() # Re-check for EULA
            else:
                messagebox.showerror("Error", "Invalid OTP. Access Denied.", parent=otp_win)

        btn = ttk.Button(otp_win, text="Verify", command=verify)
        btn.pack(pady=5)
        
        # UX Accessibility: Enter Key
        otp_win.bind('<Return>', lambda e: verify())

    def _show_eula_modal(self):
        eula_win = tk.Toplevel(self.root)
        eula_win.title("Terms & Conditions")
        eula_win.geometry("400x300")
        eula_win.transient(self.root)
        eula_win.grab_set()

        tk.Label(eula_win, text="Enterprise Terms of Use", font=("Segoe UI", 12, "bold")).pack(pady=10)
        txt = scrolledtext.ScrolledText(eula_win, width=40, height=10)
        txt.pack(padx=10, pady=5)
        txt.insert(tk.END, "By clicking Accept, you agree to the Enterprise constraints and monitoring.")
        ...
        btn = ttk.Button(eula_win, text="Accept", command=accept)
        btn.pack(pady=10)
        eula_win.bind('<Return>', lambda e: accept())
        btn.focus()

    def _init_paths(self):
        """Encapsulated path mapping for compliance and portability."""
        # Use user_dirs for all writable content
        self.config_file     = self.user_dirs['config']  / "config.json"
        self.master_log_file = self.user_dirs['logs']    / "send_logs.txt"
        self.log_file        = self.master_log_file
        self.login_file      = self.user_dirs['config']  / "login.txt"
        self.progress_file   = self.user_dirs['config']  / "send_progress.json"
        
        self._checkpoints_dir = self.user_dirs['config'] / "checkpoints"
        self._checkpoints_dir.mkdir(exist_ok=True)
        self._locks_dir = self.user_dirs['config'] / "locks"
        self._locks_dir.mkdir(exist_ok=True)
        
        self.templates_dir = self.user_dirs['exports'] / "templates"
        self._failed_dir   = self.user_dirs['exports'] / "failed"

        self.templates_dir.mkdir(exist_ok=True)
        self._failed_dir.mkdir(exist_ok=True)

    def _migrate_data(self):
        """Phase 14.2: Seamlessly move data from App Dir to %AppData% if found."""
        migration_targets = {
            self.base_dir / "config":  self.user_dirs['config'],
            self.base_dir / "logs":    self.user_dirs['logs'],
            self.base_dir / "exports": self.user_dirs['exports']
        }
        
        for old_dir, new_dir in migration_targets.items():
            if old_dir.exists() and old_dir != new_dir:
                print(f"[MIGRATION] Transferring data: {old_dir.name} -> %AppData%")
                for item in old_dir.iterdir():
                    ...
                    if not dest.exists():
                        try:
                            ...
                            print(f"[WARNING] [MIGRATION] Could not move {item.name}: {e}")
    ...
    def _setup_styles(self):
        ...
        style.configure('TLabelFrame', background=self.colors['panel'], foreground=self.colors['aqua'], 
                        font=('Segoe UI', 11, 'bold'), borderwidth=4, relief='groove')
    ...
    def _setup_main_containers(self):
        ...
        # Phase 13.5: Strictly Fixed Sidebar (200px FIX)
        self.main_frame.columnconfigure(0, weight=0, minsize=200) # Fixed Sidebar
        self.main_frame.columnconfigure(1, weight=1) # Fluid Main View
        self.main_frame.rowconfigure(1, weight=1)
        
        # Phase 13.7: Instance-level Panel Definition (Fixed-ordering guard)
        self.left_panel = ttk.Frame(self.main_frame)
        self.left_panel.grid(row=1, column=0, sticky=(tk.N, tk.S, tk.E, tk.W), padx=(0, 5))
        
        self.right_panel = ttk.Frame(self.main_frame, padding="3")
        self.right_panel.grid(row=1, column=1, sticky="nsew")
        
        # Phase 13.8: Activity Log Initialization (Crucial for early logging)
        self._setup_mission_log()
    ...
    def _setup_hud(self):
        ...
        if logo_p.exists() and PIL_SUPPORT:
            try:
        ...
        tk.Label(hud_frame, text="🚀 MISSION CONTROL: jBES Discovery", bg=self.colors['panel'], fg=self.colors['text'], 
    ...
    def _setup_sidebar(self):
        ...
        # Distribute items vertically to fit the height
        ...
        sec_frame.grid(row=0, column=0, sticky="nsew", pady=(0, 5))
        ...
        self.password_entry.bind('<Return>', lambda e: self.save_account()) # UX Accessibility Fix
        ...
        opt_frame.grid(row=1, column=0, sticky="nsew", pady=(0, 5))
        ...
        self.refresh_btn = ttk.Button(opt_frame, text="🔄 REFRESH", command=self.restart_application, style='Accent.TButton', width=12)
        self.refresh_btn.grid(row=3, column=0, columnspan=3, pady=(5, 0), sticky="ew")
        ToolTip(self.refresh_btn, "Reload application to apply code changes immediately.")
        
        # Phase 1: HTTP Relay Configuration
        ttk.Label(opt_frame, text="SendGrid:").grid(row=4, column=0, sticky="w", pady=(5,0))
        self.sendgrid_key_var = tk.StringVar(value=self.config.get('sendgrid_key', ''))
        self.sendgrid_entry = ttk.Entry(opt_frame, textvariable=self.sendgrid_key_var, show="*")
        self.sendgrid_entry.grid(row=4, column=1, columnspan=2, sticky="ew", pady=(5,0))
        ToolTip(self.sendgrid_entry, "Enter SendGrid API Key for HTTP Relay. Bypasses SMTP port 25/587 throttling.")
        # UX Binding
        self.sendgrid_entry.bind('<Return>', lambda e: self.save_config_from_ui())

        ttk.Label(opt_frame, text="ℹ️ Batch Safety Activated", font=('Segoe UI', 7, 'italic'), foreground='#00B8D4').grid(row=5, column=0, columnspan=3, sticky="w")

        # 3. Actions & Status
        mon_frame = ttk.LabelFrame(left_panel, text="📊 Monitor", padding="5")
        mon_frame.grid(row=2, column=0, sticky="nsew", pady=(0, 5))
        ...
        bt = [("📂  T-LOAD", self.load_template, 2, 0), ("💾  T-SAVE", self.save_template, 2, 1),
              ("🛑  HALT", self.stop_sending, 3, 0), ("▶ RESUME", self.resume_sending, 3, 1),
        ...
        # 4. Enterprise Support (Added item for Information & Resources)
        sup_frame = ttk.LabelFrame(left_panel, text="🏮 Support", padding="5")
        sup_frame.grid(row=3, column=0, sticky="nsew", pady=(0, 5))
        sup_frame.columnconfigure(0, weight=1)
        
        info_lines = [("Mission ID:", "jBES-DISCOVERY-6.1"), ("Status:", "READY"), ("Engine:", "v6.1-STABLE")]
        for i, (k, v) in enumerate(info_lines):
            f = ttk.Frame(sup_frame)
            f.pack(fill="x", pady=1)
            ttk.Label(f, text=k, font=('Segoe UI', 8)).pack(side="left")
            ttk.Label(f, text=v, font=('Segoe UI', 8, 'bold'), foreground=self.colors['aqua']).pack(side="right")
            
        ttk.Button(sup_frame, text="📖 MISSION DOCS", command=self._show_help_mission, style='Accent.TButton').pack(fill="x", pady=(5,0))


        # 4. Activity Log removed from here (Relocated to right of Editor per Tactical Update)
    ...
    def _setup_composition_area(self):
        ...
        # Phase 13.7: 2-Column Tactical Grid Consolidation
        right_panel.columnconfigure(0, weight=1)  # Fluid Workspace (Subject/Editor)
        right_panel.columnconfigure(1, weight=0, minsize=200) # Fixed Command Column (Launch/Log)
        right_panel.rowconfigure(2, weight=1)      # Shared Fluid Height Row
        ...
        self.from_email_combo.bind("<<ComboboxSelected>>", self.on_account_selected) # SYNC: Trigger lock on selection
        ...
        # Load Small Tactical Icon for Button
        self.btn_logo = None
        logo_p = self.dirs['assets'] / "mission_icon.png"
        if logo_p.exists() and PIL_SUPPORT:
            try:
                # Smaller icon for button (30x30)
                img = Image.open(logo_p).resize((30, 30), Image.Resampling.LANCZOS)
                self.btn_logo = ImageTk.PhotoImage(img)
            except: pass
        ...
        sub_f.grid(row=1, column=0, sticky="nsew", pady=(0, 3))
        sub_f.columnconfigure(0, weight=1)
        self.subject_var = tk.StringVar()
        
        # Tactical Border Contrast for Subject Box
        sub_border = tk.Frame(sub_f, bg=self.colors['aqua'], bd=1)
        sub_border.pack(fill="x", expand=True, padx=2, pady=2)
        tk.Entry(sub_border, textvariable=self.subject_var, font=('Segoe UI', 11), relief="flat", bd=0, highlightthickness=0).pack(fill="x", expand=True)

        
        # Launch Container: Docked above Activity Log in Fixed Column 1
        l_cont = tk.Frame(right_panel, bg=self.colors['aqua'], bd=0)
        l_cont.grid(row=1, column=1, sticky="nsew", padx=(3, 0), pady=(0, 3))
        l_inner = tk.Frame(l_cont, bg=self.colors['bg'], bd=0)
        l_inner.pack(fill="both", expand=True, padx=2, pady=2)

        self.send_btn = RoundedButton(l_inner, text="LAUNCH MISSION", command=self.start_sending, bg=self.colors['text'], 
                                      fg='#000000', height=60, corner_radius=15, font=('Segoe UI', 12, 'bold'),
                                      image=self.btn_logo)
        self.send_btn.pack(fill="both", expand=True)



    def _setup_mission_log(self):
        """Phase 13.8: Activity Log - Docked under Launch Button."""
        l_frame = ttk.LabelFrame(self.right_panel, text="📜 Activity Log", padding="3")
        l_frame.grid(row=2, column=1, sticky="nsew", pady=(0, 3))
        l_frame.columnconfigure(0, weight=1); l_frame.rowconfigure(0, weight=1)

        
        self.log_text = scrolledtext.ScrolledText(l_frame, wrap=tk.WORD, font=('Consolas', 8), width=20, bg='#D3D3D3', bd=0)
        self.log_text.grid(row=0, column=0, sticky="nsew")
        ttk.Button(l_frame, text="🗑️ PURGE", command=lambda: self.log_text.delete('1.0', tk.END), style='Accent.TButton').grid(row=1, column=0, pady=2, sticky="ew")

    def _setup_rich_editor(self):
        """Hierarchy level 3: Email Editor - Left Fluid Column."""
        right_panel = self.right_panel
        
        body_frame = ttk.LabelFrame(right_panel, text="✉️ Email Body (HTML Enabled)", padding="3")
        body_frame.grid(row=2, column=0, sticky="nsew", pady=(0, 3), padx=(0, 3))
        ...
        self.rich_editor = RichTextEditor(self.root, self.body_text, lambda: self.html_var.set(True), 
                                        ...
                                        help_mission_callback=self._show_help_mission)
    ...
    def save_config_from_ui(self):
        """Called from UI Return binding to persist API key."""
        self.config['sendgrid_key'] = self.sendgrid_key_var.get()
        self.save_config()
        self.log_message("✅ Settings saved via Keyboard Enter.")
    ...
    def save_config(self):
        ...
        if hasattr(self, 'sendgrid_key_var'):
            self.config['sendgrid_key'] = self.sendgrid_key_var.get()
    ...
    def on_account_selected(self, event=None):
        """Auto-fill password and enforce SINGLE-INSTANCE Mission Lock."""
        # Detect which widget triggered the event
        widget = event.widget if event else None
        if widget == self.from_email_combo:
            selected_email = self.from_email_combo.get().strip()
            self.account_email_var.set(selected_email) # SYNC Sidebar
        else:
            selected_email = self.account_email_var.get().strip()
            self.from_email_combo.set(selected_email)  # SYNC Top Strip
            
        if not selected_email: return

        
        # 1. Release previous mission lock if switching
        if self._active_lock_path:
            self._release_account_lock()
            
        # 2. Acquire new mission lock for targeted account
        success, msg = self._acquire_account_lock(selected_email)
        if not success:
            messagebox.showerror("MISSION CONFLICT", msg)
            self.account_email_var.set("") # Reset selection
            self.password_var.set("")
            return

        # 3. Dedicated Log Integrity (Account-Specific logs)
        safe_email = re.sub(r'[^a-zA-Z0-9]', '_', selected_email)
        self.log_file = self.dirs['logs'] / f"mission_{safe_email}.log"
        self.log_message(f"📑 Mission Log Separated: {self.log_file.name}")

        if selected_email in self.accounts:
            self.password_var.set(self.accounts[selected_email])
            self.log_message(f"🔑 Identity Verified: {selected_email}")

    def _acquire_account_lock(self, email):
        """Standard singleton check to prevent dual-instance mission collisions."""
        safe_email = re.sub(r'[^a-zA-Z0-9]', '_', email)
        lock_path = self._locks_dir / f"{safe_email}.lock"
        ...
        if lock_path.exists():
            try:
                ...
                if self._is_pid_alive(old_pid):
                    return False, f"Account '{email}' is currently ACTIVE in another Mission Console (PID: {old_pid}).\n\nOnly one instance per email is permitted to ensure transmission integrity."
            except: pass # Lock is stale or corrupt
        ...
        try:
            ...
            return False, f"Internal Lock Failure: {e}"

    def _release_account_lock(self):
        """Flush the mission lock from the filesystem."""
        if self._active_lock_path and self._active_lock_path.exists():
            try: self._active_lock_path.unlink()
            except: pass
        self._active_lock_path = None

    def _is_pid_alive(self, pid):
        """Cross-platform check for active process existence."""
        try:
            if os.name == 'nt':
    ...
    def _on_app_exit(self):
        """Hierarchy level 1: Safely terminate all mission resources before shutdown."""
        self._release_account_lock()
        self.root.destroy()
...
"""
    ...
    def start_sending(self):
        ...
        security_email = self.account_email_combo.get().strip()
        if not sender_email:
            messagebox.showerror("Error", "Please select or enter a sender email!")
            return

        if sender_email.lower() != security_email.lower():
            messagebox.showerror("Error", "Please use same email and account to send the mail")
            return
    ...
    def _smtp_connect(self, email, password):
        """Single SMTP factory: SSL or STARTTLS with Socket Buffer Tuning (v6.7)."""
        host, port = self._smtp_host_for(email)
        self.log_message(f"📡 Initiating Connection → {host}:{port}")
        ...
        try:
            ...
            # --- TCP TACTICAL TUNING (Gap: 100Mbps+ WiFi Optimization) ---
            # Increase Send Buffer to 1MB (1024 * 1024) to handle attachments in fewer 'DATA' chunks
            try:
                server.sock.setsockopt(socket.SOL_SOCKET, socket.SO_SNDBUF, 1048576)
                self.log_message(f"🚀 Link Optimized: 1MB Buffer Ready")
            except: pass
            
            server.login(email, password)
            self.log_message(f"✅ Link Established: {email}")
            return server
        except Exception as e:
            self.log_message(f"❌ Handshake failed: {e}")
            raise

    def _build_account_cycle(self, primary_email):
        """Build itertools.cycle using ONLY the primary sender (1 session 1 email id)."""
        keys = [k for k in self.accounts]
        primary = next((k for k in keys if k.lower() == primary_email.lower()), None)
        if primary and self.accounts.get(primary):
            pairs = [(primary, self.accounts[primary])]
        else:
            pairs = [(primary_email, '')]
        self._account_cycle = itertools.cycle(pairs)

    def _smtp_send_with_retry(self, server, msg, account_pair, max_retries=2):
        """Send with auto-reconnect on connection loss. (Gap 11: Persistent Mode)."""
        for attempt in range(max_retries):
            try:
                ...
                return server
            except (smtplib.SMTPServerDisconnected, ConnectionResetError, BrokenPipeError, OSError, smtplib.SMTPResponseException) as exc:
                self.log_message(f"🔄 Socket Reset (Attempt {attempt + 1})... [{exc}]")
                try: server.quit() 
                except: pass
                server = None # Force reconnect on next attempt
        raise smtplib.SMTPException("Mission Control: Server unreachable after retries.")
    ...
    def _post_send_ops(self, recipient, idx, sent, log_fh, failed_list):
        ...
        # Log entry is now handled in unified loop (v6.5) to include High-Res metrics
        pass

        # Progress checkpoint & Disk sync every 50 successes (Optimized from 10 to reduce disk I/O)
        if sent % 50 == 0:
    ...
    def send_emails_thread(self, start_idx=0):
        ...
        try:
            ...
            # --- Phase 1: High-Speed Attachment Preamble (Zip Compression) ---
            pre_processed_attachments = []
            api_attachments = [] # specifically for SendGrid
            for file_path in self.attachments:
                try:
                    p = Path(file_path).absolute()
                    # Phase 1: Dynamic memory compression
                    zip_name, file_data, is_zipped = ZipManager.compress_to_memory(str(p), threshold_mb=1.0)
                    
                    # Store raw API attachments for SendGrid payload
                    api_attachments.append({'filename': zip_name, 'data': file_data})

                    # Create atomic part for SMTP
                    part = MIMEBase('application', 'octet-stream')
                    part.set_payload(file_data)
                    encoders.encode_base64(part)
                    part.add_header('Content-Disposition', f'attachment; filename="{zip_name}"')
                    # Set a dedicated flag to prevent re-encoding in the loop
                    pre_processed_attachments.append(part)
                    comp_msg = " [ZIPPED]" if is_zipped else ""
                    self.log_message(f"📦 Pre-encoded: {zip_name} ({len(file_data)/1024:.0f} KB){comp_msg}")
                except Exception as ae:
                    self.log_message(f"⚠️ Attachment error {file_path}: {ae}")
            ...
            with open(self.log_file, 'a', encoding='utf-8') as log_fh:
                for idx in range(start_idx, total):
                    ...
                    # --- Connection Refresh Boundary: refresh connection to bypass throttling (Gap 39: Optimized) ---
                    steps = idx - start_idx
                    # CONNECTION REFRESH: Refresh connector every 10 emails instead of 50 for max throughput
                    if steps > 0 and steps % 10 == 0:
                        try: server.quit()
                        except: pass
                        
                        account_pair = next(self._account_cycle)
                        self.log_message(f"🔄 Connection Refresh — Avoiding Throttling → {account_pair[0]}")
                        
                        # Flush CSV cache at rotation boundary
                        if hasattr(self, 'current_filename'):
                            EmailChecker.flush_cache(self.current_filename)
                        
                        # No sleep here unless explicitly requested (already handled by delay_var)
                        try:
                            server = self._smtp_connect(*account_pair)
                        except Exception as stop_e:
                            self.log_message(f"⚠️ Connection refresh failed ({stop_e}) — retrying")
                            account_pair = next(self._account_cycle)
                            try:
                                server = self._smtp_connect(*account_pair)
                            except Exception as stop_e2:
                                self.log_message(f"❌ Aborting mission: {stop_e2}")
                                break
                    
                    # BATCH PAUSE: Maintain safety pause every 50 emails
                    if steps > 0 and steps % batch_limit == 0:
                        self.log_message(f"⏸️ Batch safety pause ({batch_pause:.0f}s)...")
                    ...
                    try:
                        ...
                        # --- Phase 2: Transmission (API Relay & SMTP Fallback) ---
                        s_send = time.time()
                        sendgrid_key = self._snap.get('sendgrid_key', '')
                        api_sent = False
                        
                        if sendgrid_key:
                            try:
                                engine = APIRelayEngine(api_key=sendgrid_key)
                                to_email_clean = recipient['email'].replace(';', ',')
                                engine.send_via_sendgrid(
                                    sender_email=account_pair[0], 
                                ...
                                log_fh.write(f"[{datetime.now().strftime('%H:%M:%S')}] [API RELAY] {recipient['email']} - SUCCESS [Build: {t_build:.2f}s | Data: {t_send:.2f}s]\n")
                            except Exception as api_err:
                                err_msg = str(api_err)
                                if "HTTP_429_RATE_LIMIT" in err_msg:
                                    self.log_message("⚠️ SendGrid Rate Limit (429) hit. Falling back to SMTP pool.")
                                elif "MISSING_DEPENDENCY" in err_msg:
                                    self.log_message("⚠️ API Relay disabled ('requests' missing). Falling back to SMTP.")
                                else:
                                    self.log_message(f"⚠️ API Relay failed: {err_msg}. Falling back to SMTP pool.")
                                # Fallback natively to SMTP (api_sent remains False)
                        if not api_sent:
                            server = self._smtp_send_with_retry(server, msg, account_pair)
                            t_send = time.time() - s_send
                            log_fh.write(f"[{datetime.now().strftime('%H:%M:%S')}] [SMTP] {recipient['email']} - SUCCESS [Build: {t_build:.2f}s | Data: {t_send:.2f}s]\n")
                            
                        # End Phase 2 Transmission
                        success = True
                        sent += 1
                        self.log_message(f"✉️ Sent {idx+1}/{total} → {recipient['email']} [Build:{t_build:.2f}s | Data:{t_send:.2f}s]")

                        # --- Phase 3: Post-Send Ops ---
                        s_post = time.time()
                        self._post_send_ops(recipient, idx, sent, log_fh, failed_list)
                        t_post = time.time() - s_post
                        
                        # Consolidated Atomic Log (v6.5)
                        try:
                            log_fh.write(f"[{datetime.now()}] SUCCESS - {recipient['email']} [Build:{t_build:.4f}s | Data:{t_send:.4f}s | Log:{t_post:.4f}s]\n")
                            log_fh.flush()
                        except: pass
                        if t_post > 1.0:
    ...
    def log_message(self, message):
        ...
        # 1. Real-time Widget Update
        def _insert():
            self.log_text.insert(tk.END, msg_line)
            self.log_text.see(tk.END)
            try:
                if int(float(self.log_text.index('end-1c'))) > 5000:
                    self.log_text.delete('1.0', '2.0')
            except: pass

        self.root.after(0, _insert)
        
        # 2. Separate Log Integrity (File Persistence)
        try:
            # Use append mode to maintain history within the session log
            with open(self.log_file, 'a', encoding='utf-8') as f:
                f.write(msg_line)
        except:
            pass # Silently fail file-log to prevent app crash if disk is locked
